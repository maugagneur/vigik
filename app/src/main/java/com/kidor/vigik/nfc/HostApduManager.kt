...
class HostApduManager @Inject constructor(@ApplicationContext context: Context) {
    ...
    private val broadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action.equals(VigikHostApduService.APDU_SERVICE_ACTION_NEW_MESSAGE)) {
                when (val type = intent?.getStringExtra(VigikHostApduService.KEY_MESSAGE_TYPE)) {
                    VigikHostApduService.TYPE_APDU_COMMAND -> {
                        val apduCommand = intent.getByteArrayExtra(VigikHostApduService.KEY_APDU_COMMAND)
                        // Notify listeners
                        listeners.forEach { it.onApduCommandReceived(apduCommand) }
                    }
                    VigikHostApduService.TYPE_DEACTIVATED -> {
                        val reason = intent.getIntExtra(VigikHostApduService.KEY_DEACTIVATED_REASON, -1)
                        // Notify listeners
    ...
    init {
        ...
        val intentFilter = IntentFilter(VigikHostApduService.APDU_SERVICE_ACTION_NEW_MESSAGE)
        localBroadcastManager.registerReceiver(broadcastReceiver, intentFilter)

        // Connect to the custom host APDU service
        context.bindService(
            Intent(context, VigikHostApduService::class.java),
            serviceConnection,
    ...
    fun sendApduResponse(apduResponse: ByteArray?) {
        val message = Message.obtain(null, VigikHostApduService.MSG_RESPONSE_APDU).apply {
            data = Bundle().apply {
                putByteArray(VigikHostApduService.KEY_DATA, apduResponse)
            }
        }
